...
from env.CoinTraderEnv import OhlcvEnv
...
class OpenAIGym(Environment):
    ...
    #DPG
    def set_dpg_actions(self):
        self._actions = self.actions_ = dict(type='float', shape=3)
        return self._actions
    ...
    def execute(self, actions):
        if self.visualize:
            self.gym.render()
        action = OpenAIGym.unflatten_action(action=actions)
...
def create_btc_env(infoname, window_size, path, ep_len, train):
    raw_env = OhlcvEnv(infoname=infoname, window_size=window_size, path=path, ep_len = ep_len,train=train)
