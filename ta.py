...
def hhv(s, n):
    return s.rollling(n).max() # moments.rolling_max(s, n)


@series_indicator('low')
def llv(s, n):
    return s.rolling(n).min() #moments.rolling_min(s, n)


@series_indicator('close')
def ema(s, n, wilder=False):
    span = n if not wilder else 2*n - 1
    return s.ewm(span=span).mean() #moments.ewma(s, span=span)
...
def aroon(s, n=25):
    up = 100 * s.high.rolling(n+1).apply(lambda x: x.argmax()) / n #moments.rolling_apply(s.high, n + 1, lambda x: x.argmax()) / n
    dn = 100 * s.low.rolling(n+1).apply(lambda x: x.argmin()) / n # moments.rolling_apply(s.low, n + 1, lambda x: x.argmin()) / n
...
def stoch(s, nfastk=14, nfullk=3, nfulld=3):
    ...
    fullk = fastk.rolling(nfullk).mean() #moments.rolling_mean(fastk, nfullk)
    fulld = fullk.rolling(nfulld).mean() #moments.rolling_mean(fullk, nfulld)
...
def cci(s, n=20, c=0.015):
    ...
    mavg = s.rolling(n).mean() #moments.rolling_mean(s, n)
    mdev = s.rolling(n).apply(lambda x: np.fabs(x-x.mean()).mean()) # moments.rolling_apply(s, n, lambda x: np.fabs(x - x.mean()).mean())
...
def cmf(s, n=20):
    ...
    return clv*vol.rolling(n).sum() / vol.rolling(n).sum() #moments.rolling_sum(clv*vol, n) / moments.rolling_sum(vol, n)
...
def kst(s, r1=10, r2=15, r3=20, r4=30, n1=10, n2=10, n3=10, n4=15, nsig=9):
    rocma1 = (s / s.shift(r1) - 1).rolling(n1).mean() #moments.rolling_mean(s / s.shift(r1) - 1, n1)
    rocma2 = (s / s.shift(r2) - 1).rolling(n2).mean() #moments.rolling_mean(s / s.shift(r2) - 1, n2)
    rocma3 = (s / s.shift(r3) - 1).rolling(n3).mean() #moments.rolling_mean(s / s.shift(r3) - 1, n3)
    rocma4 = (s / s.shift(r4) - 1).rolling(n4).mean() #moments.rolling_mean(s / s.shift(r4) - 1, n4)

    kst = 100*(rocma1 + 2*rocma2 + 3*rocma3 + 4*rocma4)
    sig = kst.rolling(nsig).mean() #moments.rolling_mean(kst, nsig)
...
def ultimate(s, n1=7, n2=14, n3=28):
    ...
    avg1 = bp.rolling(n1).sum() / tr.rolling(n1).sum() #moments.rolling_sum(bp, n1) / moments.rolling_sum(tr, n1)
    avg2 = bp.rolling(n2).sum() / tr.rolling(n2).sum() #moments.rolling_sum(bp, n2) / moments.rolling_sum(tr, n2)
    avg3 = bp.rolling(n3).sum() / tr.rolling(n3).sum() #moments.rolling_sum(bp, n3) / moments.rolling_sum(tr, n3)
...
def auto_envelope(s, nema=22, nsmooth=100, ndev=2.7):
    ...
    csize = mdiff.ewm(nsmooth)*ndev #moments.ewmstd(mdiff, nsmooth)*ndev
...
def bbands(s, n=20, ndev=2):
    mavg = s.rolling(n).mean() #moments.rolling_mean(s, n)
    mstd = s.rolling(n).std() #moments.rolling_std(s, n)
...
def safezone(s, position, nmean=10, npen=2.0, nagg=3):
    ...
    pen = sgn*s.diff().rolling(nmean).apply(lambda x: x[x > 0].mean() if (x > 0).any() else 0)
    # moments.rolling_apply(
    #     sgn*s.diff(), nmean,
...
def vortex(s, n=14):
    ...
    trn = tr.rolling(n).sum() #moments.rolling_sum(tr, n)

    vmp = np.abs(s.high - ss.low)
    vmm = np.abs(s.low - ss.high)

    vip = vmp.rolling(n).sum() / trn #moments.rolling_sum(vmp, n) / trn
    vin = vmm.rolling(n).sum() / trn #moments.rolling_sum(vmm, n) / trn
